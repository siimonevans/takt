...
function takt() {
    ...
    function timer() {
        ...
        function incrementTimer() {
            ...
            document.title = jquery('.timer .minutes').text() + ':' + jquery('.timer .seconds').text();
        }
        ...
        function eventHandler() {
            ...
            $taskForm[0].addEventListener('submit', function (e) {
                ...
                jquery('.current-task__data').html('<div>' + $taskLabel[0].value + '</div><div>' + $taskProject[0].value + '</div>');
            });

            // Remove task
            jquery(document).on('click', '.time-list li .delete-task', function () {
                var item = jquery(this);
                deleteItem(item);
            });

            // Edit task
            jquery(document).on('click', '.time-list li .edit-task', function () {
                var item = jquery(this);
                prepareModal(item);
            });
...
function timesheet() {
        ...
        if (localStorage.getItem('storageString')) {
            ...
            if (arrayLength !== 0) {
                for (var i = 0; i < arrayLength; i++) {
                    ...
                    if (minutes == 0) {
                        jquery('.time-list').append('<li><div>Task name: <span>' + data[i].title + '</span></div><div>Task project: <span>' + data[i].project + '</span></div><div>Task duration: <span>' + seconds + ' seconds</span></div></li>');
                    } else {
                        jquery('.time-list').append('<li><div>Task name: <span>' + data[i].title + '</span></div><div>Task project: <span>' + data[i].project + '</span></div><div>Task duration: <span>' + minutes + ' minutes and ' + seconds + ' seconds</span></div></li>');
                    }
                }
    ...
    function bindEvents() {
        if (jquery('.app--timesheet').length) {
            jquery(window).on('load', function () {
                return getData();
            });
        }
